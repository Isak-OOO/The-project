...
def main():
    ...
    class bullets(p.sprite.Sprite):
        ...
        def update(self):
            if self.rect.centerx > -10 and self.rect.centerx < SCREEN_WIDTH + 10 and self.rect.centery > -10 and self.rect.centery < SCREEN_HEIGHT + 10:
                self.rect.move_ip(self.numberx * bullet_speed, self.numbery * bullet_speed)
            else:
                self.remove()
            
            #self.rect.y += cby * bullet_speed
    ...
    while run:
        ...
        """if keys[p.K_s] or keys[p.K_LEFT]:
            change_x *= 0.95
            change_y *= 0.95"""
        speed_factor = 0.2
        change_x *= 0.99
